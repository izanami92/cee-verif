...
Évolutions de la roadmap implémentées et testées. La plupart sont mergées sur main ; les entrées du chantier ADR-015 (« Collision A1 levée », « Filet anti-faux-conforme : surface non ventilable », « Grain cellule LED », « Bâtiments par adresse fiabilisés ») sont livrées sur branches **non encore mergées**, EMPILÉES (cf. `pending-todos.md` §TODO #22). L'étape 4b (LED chantier depuis `cellules[]`) est **débloquée** (la révision étape 3 « Philo 2 » est livrée, `d589c69`), pas encore implémentée. Distinct du §7 (corrections de bugs).
