...
Évolutions de la roadmap implémentées et testées. La plupart sont mergées sur main ; les entrées du chantier ADR-015 (« Collision A1 levée » et « Filet anti-faux-conforme : surface non ventilable ») sont livrées sur branches **non encore mergées** (cf. `pending-todos.md` §TODO #22). Distinct du §7 (corrections de bugs).
