...
> ✅ **#27 LATRILLE — 1 adresse = 1 chantier + appariement au grain chantier (01–03/07/2026, branche `fix/27-decoupage-parcelle`, NON mergée)**. Règle métier **tranchée** : **1 adresse = 1 chantier** ; plusieurs bâtiments/cellules à une même adresse = toujours **1 audit + 1 synthèse** ; **parcelle ET secteur hors découpage**. Changements de comportement (les seuls de ce chantier qui touchent une gravité) :
...
> Preuve : banc d'essai `#27` + harnais `test-batiments` 36/36 & `test-familles` 70/70, preview OK. **Prochaine session : LED DELEFORTRIE (étape 4b, « 52 vs 66 » check_09d).**
