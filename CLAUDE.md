...
```
cee-verif/
├── index.html                    ← Interface + rendu + handlers (5669 lignes après phase B)
├── utils-comparaison.js          ← Module 1 (ADR-016) : 21 fonctions pures comparaison/normalisation/parsing
├── regroupement.js               ← Module 2 : attestations/cellules → chantiers (matching ADR-011, LED 4b)
├── detecteurs-alertes.js         ← Module 3 : détecteurs métier des alertes confirmables (1.1→1.5, Règles A/B)
├── io.js                         ← Module 4 : apiFetch (auth K1) + extraction PDF + gate NAF
├── moteur-checks.js              ← Module 5 : generateChecks ENTIÈRE + buildMessageAuditeur + FICHES_TECHNIQUES
...
├── test-batiments.mjs            ← Harnais bâtiments/adresses (Node, 36/36 — lit utils-comparaison.js)
├── lib/
│   └── auth.js                   ← requireAuth (Bearer APP_PASSWORD, fail-closed) — LOT 1
├── api/
...
**Dernière mise à jour** : 9 juillet 2026 — **TODO #3 : LOTs 1-4 MERGÉS sur `main` (tip `dda6670`) + PHASE B (modularisation) EXTRACTION FAITE.** Branche `feat/phase-b-modularisation` (non mergée) : ADR-016 (`4fc6153`, technique A scripts classiques, cible B = conversion ES6 en chantier futur) puis extraction PURE byte-identique en 5 commits — M1 `utils-comparaison.js` 21 fonctions + harnais re-routé (`bdbff3a`), M2 `regroupement.js` 5 fonctions (`bb24f75`), M3 `detecteurs-alertes.js` 9 fonctions (`449246c`), M4 `io.js` 4 fonctions async (`2f99ccb`), M5 `moteur-checks.js` avec `generateChecks` ENTIÈRE (`73b5566`). **index.html 8204 → 5669 lignes.** Preuves à chaque commit : banc d'identité byte-à-byte (rejeu intégral pour M5) + harnais 71/71 & 36/36 + preview validée IZANAMI (anchors). Décisions IZANAMI 09/07 : étapes 6 (encapsulation 4 globales) & 7 (orchestrateur mince) **DIFFÉRÉES à la conversion ES6** ; CSP stricte différée (retrait onclick) ; code mort intact (#35). **Reste : MERGE (geste IZANAMI).** État complet → `docs/pending-todos.md` §TODO #3 + ADR-016. *(ligne historique ci-dessous)*
**Dernière mise à jour (historique)** : 8 juillet 2026 — **TODO #3 en cours** : audit phase A FAIT + LOT 1 (`c197376`) & LOT 2 (`41a8f48`) MERGÉS sur `main`. **LOT 3 + LOT 4 LIVRÉS sur la branche `fix/m4-led-miss-signal` (mergée depuis, tip `e6ab7ed`), validés preview le 08/07** : LOT 3 = M4 (`0e28ee2` filet `check_09d_miss`), M5 (`e5937be` `Promise.allSettled`), M1 (`910d5bb` `escapeHtml` anti-XSS 13 sinks) ; LOT 4 durcissement = 4.1 erreurs non renvoyées au client (`424509d`), 4.2 timeout `AbortController` `apiFetch`/login (`351eb6f`, 30 s / analyse 120 s), 4.3 SRI pdf.js + 4 en-têtes HTTP `vercel.json` (`e6ab7ed`; CSP écartée → phase B). **Reste : MERGE (geste IZANAMI) → phase B modularisation (ADR de découpage AVANT code).** ⚠️ **Auto-deploy Vercel à la traîne (08/07)** : `vercel deploy` manuel utilisé pour les previews ; prévoir `vercel deploy --prod` après merge si besoin. **État complet → `docs/pending-todos.md` §TODO #3 (footer).** *(ligne historique ci-dessous)*
